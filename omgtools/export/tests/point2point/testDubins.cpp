...
int main()
{
    ...
    omg::Dubins* vehicle = new omg::Dubins();
    ...
    vector<vector<double>> state_trajectory(trajectory_length, vector<double>(3));
    ...
    for (int i=0; i<3*n_iter; i++){
        ...
        for (int j=0; j<trajectory_length; j++){
            ...
            converter_state >> data_state[i/3][j][k];
        }
        k++;
        if (k == 3){
            k = 0;
